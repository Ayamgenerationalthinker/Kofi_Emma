...
import type { ReminderSettingsRecord } from "../lib/localDb";

// Foreground-only local scheduling. This is an honest implementation of
// what a browser tab can actually guarantee — there is no reliable way for
// a closed tab to fire a notification at an exact time without a push
// subscription and a server, which this app deliberately does not have.
// The Settings page states this constraint explicitly.
export function useNotificationScheduler(settings: ReminderSettingsRecord | null) {
  useEffect(() => {
