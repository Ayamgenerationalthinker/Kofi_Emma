...
import { suggestNextBpm, buildBpmLadder } from "./bpmProgressionService.js";

const bounds = { minimumBpm: 60, maximumBpm: 160 };

describe("suggestNextBpm", () => {
  it("increases tempo by 5 when accuracy is excellent (>=95)", () => {
    const result = suggestNextBpm({ currentBpm: 120, accuracy: 96, ...bounds });
    expect(result.nextBpm).toBe(125);
  });

  it("increases tempo by 3 for strong but not excellent accuracy (90-94)", () => {
    const result = suggestNextBpm({ currentBpm: 120, accuracy: 92, ...bounds });
    expect(result.nextBpm).toBe(123);
  });

  it("holds tempo steady for accuracy in the 80-89 band", () => {
    const result = suggestNextBpm({ currentBpm: 120, accuracy: 85, ...bounds });
    expect(result.nextBpm).toBe(120);
  });

  it("drops tempo by 5 for accuracy in the 70-79 band", () => {
    const result = suggestNextBpm({ currentBpm: 120, accuracy: 74, ...bounds });
    expect(result.nextBpm).toBe(115);
  });

  it("drops tempo by 10 when accuracy is below 70", () => {
    const result = suggestNextBpm({ currentBpm: 120, accuracy: 60, ...bounds });
    expect(result.nextBpm).toBe(110);
  });

  it("never exceeds the exercise maximum BPM", () => {
    const result = suggestNextBpm({ currentBpm: 158, accuracy: 99, ...bounds });
    expect(result.nextBpm).toBe(160);
  });

  it("never drops below the exercise minimum BPM", () => {
    const result = suggestNextBpm({ currentBpm: 65, accuracy: 40, ...bounds });
    expect(result.nextBpm).toBe(60);
  });
});
