...
import type { CurriculumState, ProgressSummary, DailyLesson, ExerciseDetail } from "../lib/types";

function buildCoachMessage(progress: ProgressSummary | null, lesson: DailyLesson | null): string {
  ...
  if (!progress || !progress.hasAnyData) {
...
export function Dashboard() {
  const state = useFetch<CurriculumState>("/curriculum/state");
  const progress = useFetch<ProgressSummary>("/progress");
  const lesson = useFetch<DailyLesson>("/practice/today");
  const currentExercise = useFetch<ExerciseDetail>(
    state.data?.currentExerciseId ? `/exercises/${state.data.currentExerciseId}` : null,
    [state.data?.currentExerciseId]
  );

  if (state.loading || progress.loading || lesson.loading) return <LoadingState label="Loading your dashboard..." />;
  if (state.error) return <ErrorState message={state.error.message} onRetry={state.refetch} />;
  if (progress.error) return <ErrorState message={progress.error.message} onRetry={progress.refetch} />;
  if (!state.data || !progress.data) return <EmptyState message="Complete your first practice session to see your progress." />;

  const currentPhase = state.data.phases.find((p) => p.number === state.data!.currentPhaseNumber);
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-gold-600/30 bg-gradient-to-br from-charcoal-900 to-charcoal-800 p-6">
        ...
        <p className="mt-3 max-w-2xl text-parchment/70">{buildCoachMessage(progress.data, lesson.data)}</p>
      ...
      <section className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        <StatTile icon={<Gauge className="h-5 w-5" />} label="Best BPM" value={progress.data.bestBpm || "—"} />
        <StatTile icon={<Percent className="h-5 w-5" />} label="Accuracy" value={progress.data.hasAnyData ? `${progress.data.averageAccuracy}%` : "—"} />
        <StatTile icon={<Flame className="h-5 w-5" />} label="Streak" value={`${progress.data.streak.currentStreak}d`} />
        <StatTile icon={<Clock className="h-5 w-5" />} label="Minutes" value={progress.data.totalMinutesPracticed} />
        <StatTile icon={<Trophy className="h-5 w-5" />} label="Mastered" value={progress.data.masteredExercises} />
        <StatTile icon={<Lock className="h-5 w-5" />} label="Locked" value={progress.data.lockedExercises} />
      </section>
      ...
      {currentExercise.data && (
        <section className="rounded-xl border border-charcoal-700 bg-charcoal-900/50 p-4">
          ...
          <p className="mt-1 text-xl font-bold">{currentExercise.data.name}</p>
          <p className="mt-1 text-sm text-parchment/60">
            Target: {currentExercise.data.targetBpm} BPM &middot; Minimum accuracy: {currentExercise.data.minimumAccuracy}%
          </p>
          <Link to={`/curriculum/${currentExercise.data.id}`} className="mt-3 inline-block text-sm font-medium text-gold-400 hover:underline">
